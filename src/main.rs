...
struct Cli {
    ...
    /// Tokenizer source: HuggingFace model ID (e.g. "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B")
    /// or local directory path. If omitted, fetches via GetTokenizer RPC.
    #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    let loaded = if let Some(ref source) = cli.tokenizer {
        eprint!("Loading tokenizer from {source}... ");
        tokenizer::load_tokenizer_from_path_or_hub(source)
            .await
            .with_context(|| format!("failed to load tokenizer from {source}"))?
    } else {
        ...
        tokenizer::load_tokenizer_from_rpc(tokenizer_stream)
            .await
